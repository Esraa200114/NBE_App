...
const DrawerProfileCard = () => {
    ...
    const [user, setUser] = useState<{ userName: string, mobileNumber: string } | null>(null);

    useEffect(() => {
        const fetchLoggedIn = async () => {
            const storedValues = await getLoggedIn();
            if (storedValues) {
                setUser({ userName: storedValues.userName, mobileNumber: storedValues.mobileNumber });
            }
        };
